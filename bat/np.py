...
def benchmark_result(task, ltype):
    base_file = "noreading/{0}/task_{1}_{2}.txt"
    v = 0.0
    for iter in range(3):
...
if __name__ == "__main__":
    f = open("./noreading/res.csv", "w")
    f.write("tasks,rcu,mutex,comutex,smutex,cosmutex\n")
    task = 1
    for i in range(8):
        ...
        f.write(
            f"{task},{rcu},{mutex},{comutex},{smutex},{cosmutex}\n")
        task = task * 2
    f.close()
